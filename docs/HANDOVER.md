...
- **Designomgång (2026-06-20)** — UX-pass byggt, verifierat via `?demo` och mergat till main ([PR #40](https://github.com/antegj-spec/lagerassistent/pull/40)):
  - **Hem**: progressiv anteckningsform (fält fälls ut vid fokus), "Idag" flyttad till Anteckningar → kortare startskärm.
  - **Not-/plan-kort "Alt A"**: prioritet som färgad vänsterkant (bara `hög`), kategori som färgad text, deadline som diskret meta med klock-ikon, "lugnt läge" (tysta kort) för icke-brådskande. `rCard`/`rTaskListRow`.
  - **Körjournal**: bara pågående resa + luckor ("⚠ ATT FYLLA I") visas; tidigare resor + filter bakom utfällbar "Visa tidigare resor"-knapp (`<details class="cj-past">`). `rCarJournal`.
  - **Info**: "pinnad"-statusen borttagen ur UX:en → koncept **publicerad / förslag**. Huvuddelen visar bara publicerade artiklar per kategori; förslag i egen sektion; admin "⬆ Flytta till huvuddelen" (`publishInfoArticle`/`unpublishInfoArticle`). **DB-kolumnen `is_pinned` behålls internt** (true = publicerad) — ingen migration, kan döpas om till `is_published` senare.
  - **Material-detalj**: historiken bakom utfällbar "🕓 Visa historik"-knapp (`<details class="mat-history">`). Rikare vyn (totalt antal / status-fördelning / reserverat-uthyrt) behölls.
  - **Returer**: formuläret ombyggt till **rad-per-material** (material/antal/kommentar, allt fri text). Ny `return_items`-tabell (**migration 030**). Gamla returer visas via legacy-fallback (`content`/`comment`).
  - **Dev-verktyg (behålls)**: `?demo`-läge (`src/legacy/devSeed.ts`, laddas sist i index.html) seedar mock-data utan login/Supabase för visuell verifiering. `main.js` hoppar över service worker i demo-läge. Se minnesnoteringen `dev_demo_mode.md`.
...
├── migrations/             # 001-030 — körs manuellt via SQL Editor
└── docs/
...
001-030 i `migrations/`. När ny migration behövs: skriv som `migrations/0NN_name.sql` med tydlig rubrik + `-- ROLLBACK:` kommentar längst ner.

**OBS:** `supabase db push` används INTE i detta projekt — Supabase-CLI:n används bara för Edge Functions (`supabase/functions/`). Det finns ingen `supabase/migrations/`-mapp och ingen `config.toml`. Migrationer appliceras genom att klistra in SQL:en i Supabase Dashboard → SQL Editor → Run. Kör alltid migrationen INNAN ny kod deployas.

Senaste migrationer av vikt:
- **028_material_allocations** — `material_allocations` (per-post reserverat/uthyrt) + RPC:er `create_allocation`/`promote_allocation`/`close_allocation` som håller `material_counts` i synk atomiskt.
- **030_return_items** — `return_items` (rad-per-material i en retur, allt fri text). Krävs för det nya retur-formuläret.
...
- `info_articles.is_pinned` betyder numera **publicerad** (true = i huvuddelen, false = förslag). Den synliga "pinnad"-statusen är borttagen ur UX:en men kolumnen är kvar — döp ev. om till `is_published` i en framtida migration.
- `return_items` (migration 030): `material`/`quantity`/`comment` är ALLA fri text (`quantity` är text, inte numerisk — "40 m", "ca 20"). Ingen FK mot `materials_v2` (returer är en fri logg, medvetet frikopplade från lagersiffrorna). Gamla returer har data i `returns.content`/`comment` istället (legacy-fallback i `rReturCard`).
